...
| 合同交付物 | 项目文件 |
| --- | --- |
| 系统源代码 | `backend/`、`frontend/`、`qa/` |
| 数据库设计文档 | `docs/数据库设计文档.md`、`docs/数据库设计文档.docx` |
| 接口文档 | `docs/接口文档.md`、`docs/接口文档.docx` |
| 部署文档 | `docs/RIver_AGI_部署文档.docx` |
| 运维手册 | `docs/运维手册.md`、`docs/RIver_AGI_运维手册.docx` |
| 用户操作手册 | `docs/用户操作手册.md`、`docs/用户操作手册.docx` |
| 培训材料 | `docs/培训材料.md`、`docs/RIver_AGI_培训材料.docx` |
| 测试报告 | `qa-results/acceptance-20260804-112610/` |

## 19. 许可证

## 20. 合同 7.1-7.5 项目管理交付件

以下文件为待双方填写确认的项目管理模板，不代表相关会议或周报已经发生：

- `docs/项目启动纪要_模板.docx` / `.md`：启动会目标、范围、团队和沟通机制。
- `docs/详细项目计划_模板.docx` / `.md`：里程碑、任务分解、资源、风险和变更控制。
- `docs/项目周报月报_模板.docx` / `.md`：周报、月报、风险和甲方协调事项。
- `docs/项目会议纪要_模板.docx` / `.md`：周例会、里程碑评审和阶段总结会议记录。
- `docs/问题升级机制与台账_模板.docx` / `.md`：3 个工作日升级规则、等级和问题台账。

模板填写后，应由双方项目负责人确认；正式会议纪要应在会议结束后 2 个工作日内归档，周报/月报应按合同周期提交。

## 21. 合同 9.1 全套交付物

完整交付物映射见 `docs/交付物总目录.md`，覆盖：

- 需求规格说明书、需求变更记录。
- 系统架构、数据库、接口、UI 设计资料。
- 前端、Java、Python、数据库脚本、配置模板和部署脚本。
- 测试计划、测试用例、测试报告、缺陷跟踪记录。
- 部署手册、环境配置、运维手册、监控方案、应急预案。
- 用户操作手册、培训材料、常见问题。
- 项目计划、启动纪要、周报/月报、会议纪要、变更记录和验收确认单。

其中会议纪要、周报/月报、缺陷、变更和验收确认单为真实项目管理记录模板，必须在实际发生后填写并由双方确认，不以空白模板冒充已完成记录。
