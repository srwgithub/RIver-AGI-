...
public class PredictionService {
    ...
    private ModelVersion trainModel(PredictionTask task) throws Exception {
        ...
        if ("AUTO".equalsIgnoreCase(modelType)) {
            double linError = computeMAPE(series, regression);
            double expError = computeMAPE(series, expSmoothing);
            actualModelUsed = linError <= expError ? "LINEAR_REGRESSION" : "EXPONENTIAL_SMOOTHING";
        }
        
        double[] finalModel = "EXPONENTIAL_SMOOTHING".equals(actualModelUsed) ? expSmoothing : regression;
        
        double mae = 0, mse = 0, mape = 0;
        for (int i = 0; i < series.size(); i++) {
            double actual = series.get(i).value;
            double predicted = finalModel[0] + finalModel[1] * i;
        ...
        metrics.put("r2", round(rSquared(series, finalModel)));
