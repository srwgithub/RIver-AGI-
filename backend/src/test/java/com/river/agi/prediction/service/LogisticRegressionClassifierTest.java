...
class LogisticRegressionClassifierTest {
    ...
    private LogisticRegressionClassifier algorithm;
    private ObjectMapper objectMapper;

    @BeforeEach
    void setUp() throws Exception {
        objectMapper = new ObjectMapper();
        algorithm = new LogisticRegressionClassifier(objectMapper);
        setField(algorithm, "defaultWindowSize", 3);
        setField(algorithm, "learningRate", 0.1);
        setField(algorithm, "maxIterations", 50);
        setField(algorithm, "regularization", 0.001);
    }
    ...
    void metadata() {
        assertEquals("逻辑回归分类器", algorithm.getAlgorithmName());
        assertEquals("LOGISTIC_REGRESSION_CLASSIFIER", algorithm.getAlgorithmType());
    ...
    void getAlgorithmParams() {
        Map<String, Object> params = algorithm.getAlgorithmParams(task(1L, 1L));
        assertEquals("LOGISTIC_REGRESSION_CLASSIFIER", params.get("algorithm"));
        assertEquals(0.1, params.get("learningRate"));
        assertEquals(50, params.get("maxIterations"));
        assertEquals(0.001, params.get("regularization"));
    ...
    void train_binaryClass() {
        ...
        assertNotNull(mv);
        assertEquals("LOGISTIC_REGRESSION_CLASSIFIER", mv.getModelType());
        assertEquals("CLASSIFICATION", mv.getTaskType());
        assertEquals("ACTIVE", mv.getStatus());
        assertNotNull(mv.getTrainingMetricsJson());
        assertTrue(mv.getTrainingMetricsJson().contains("weights"));
        assertTrue(mv.getTrainingMetricsJson().contains("biases"));
    ...
    @Test
    @DisplayName("train with multi-class series exercises multiclass metric paths")
    ...
    void evaluateClassifier_emptySeries() {
        ...
        Map<String, Object> result = algorithm.evaluateClassifier(task(1L, 1L), new ArrayList<>(), mv);
    ...
    void evaluateClassifier_nullSeries() {
        ModelVersion mv = new ModelVersion();
        Map<String, Object> result = algorithm.evaluateClassifier(task(1L, 1L), null, mv);
        assertEquals(0.0, result.get("accuracy"));
        assertEquals("No data provided", result.get("error"));
    }
